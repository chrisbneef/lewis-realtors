#!/usr/bin/env bash
# Seamless background loop: encode-loop.sh <input> <start> <dur> <outname> [WxH] [crf]
# Cuts [start,start+dur], scales+crops to fill WxH, crossfades the tail into the
# head for a seamless loop, strips audio, and writes a poster JPG.
set -euo pipefail
IN="$1"; START="$2"; DUR="$3"; NAME="$4"; SIZE="${5:-1920x1080}"; CRF="${6:-25}"
W="${SIZE%x*}"; H="${SIZE#*x}"; XF=1
OUT="public/assets/video"; mkdir -p "$OUT"
SHIFT=$(python3 -c "print(round($DUR-$XF,3))")
ffmpeg -y -loglevel error -ss "$START" -i "$IN" -t "$DUR" -an -filter_complex \
"[0:v]setpts=PTS-STARTPTS,scale=${W}:${H}:force_original_aspect_ratio=increase,crop=${W}:${H},format=yuv420p[v0];\
[v0]split[body][head];\
...
echo "$(du -h "$OUT/$NAME.mp4" | cut -f1)  $OUT/$NAME.mp4"
